...
public class LoginServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if ("/LogoutServlet".equals(path)) {
            ...
            // Cualquier GET a /LoginServlet → envío a login.jsp
            response.sendRedirect(request.getContextPath() + "/login.jsp");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (u != null) {
            // Credenciales válidas: guardamos el Usuario completo en sesión
            HttpSession session = request.getSession();
            session.setAttribute("usuarioConectado", u);
            response.sendRedirect(request.getContextPath() + "/Menu.jsp");
        } else {
            // Falló login: devolvemos a login.jsp con mensaje de error
